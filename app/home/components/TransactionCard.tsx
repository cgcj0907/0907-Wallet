  ...
  return (
    <div className="p-4 space-y-1 caret-transparent min-h-80">
      ...
      {displayedTxs.map((tx, index) => {
        ...
        // 注意：因为我们在 displayedTxs 中 slice 了，所以 key 仍然用 hash + index 保证唯一
        return (
          <div
            key={`${tx.hash}-${index}`}
            ...
            onClick={() => setExpandedHash(expandedHash === `${tx.hash}-${index}` ? null : `${tx.hash}-${index}`)}
          >
            <div className="flex items-center justify-between">
              ...
              <div className="text-sky-500 text-sm font-medium">
                {expandedHash === `${tx.hash}-${index}` ? <i className="fa-solid fa-caret-up"></i> : <i className="fa-solid fa-caret-down"></i>}
              </div>
            </div>

            {expandedHash === `${tx.hash}-${index}` && (
