...
import { stakeToken, reedeemAtoken } from "@/app/chainInteraction/lib/aave";
...
export default function AaveForm({ scheme = 'usdc', mode }: AaveFormProps) {
  ...
  const handleRedeemSubmit = async () => {
    ...
    try {
      ...
      // 注意：这里调用的是 reedeemAtoken 函数
      const result = await reedeemAtoken(keyPath, password, tokenType, amountBigInt);
      ...
      const message = e?.message || e?.toString?.() || '赎回失败';
      setError(String(message));
      setTimeout(() => setError(undefined), 6000);
    } finally {
      setLoading(false);
        ...
        <>
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2">
              ...
              <div>
                <h3 className={`font-bold ${colors.text}`}>赎回 {tokenType.toUpperCase()}</h3>
                <p className="text-sm text-gray-600">赎回 stkToken 为底层资产</p>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                >
                  ...
                  <div className="text-left">
                    <div className="font-medium text-blue-800">USDC</div>
                    <div className="text-xs text-blue-600">赎回 aToken</div>
                  </div>
                </button>
                ...
                >
                  ...
                  <div className="text-left">
                    <div className="font-medium text-green-800">USDT</div>
                    <div className="text-xs text-green-600">赎回 aToken</div>
                  </div>
                </button>
              </div>
            </div>
            ...
            >
              <div className="flex items-center gap-2">
                ...
                <div className="text-left">
                  <div className="text-sm font-medium text-gray-800">赎回需知</div>
                  <div className="text-xs text-gray-600">点击查看赎回说明</div>
                </div>
              </div>
              <span className={`text-gray-400 ${showInfo ? 'rotate-180' : ''}`}>
                ▼
            ...
            {showInfo && (
              <div className="p-3 bg-blue-50 border border-blue-200 rounded-lg text-sm text-blue-800">
                <div className="space-y-2">
                  <div className="font-medium mb-2 text-blue-900">关于 Aave 赎回：</div>
                  <ul className="space-y-1">
                    <li className="flex items-start gap-2">
                      <span className="text-green-500 mt-0.5">↔️</span>
                      <span>赎回 stkToken 换回底层资产（USDC/USDT）</span>
                    </li>
                    <li className="flex items-start gap-2">
                      <span className="text-green-500 mt-0.5">💰</span>
                      <span>赎回时会自动结算所有应计收益</span>
                    </li>
                    <li className="flex items-start gap-2">
                      <span className="text-blue-500 mt-0.5">⚡</span>
                      <span>支持部分赎回，无需赎回全部资产</span>
                    </li>
                    <li className="flex items-start gap-2">
                      <span className="text-amber-500 mt-0.5">⏱️</span>
                      <span>赎回需要网络确认，请耐心等待</span>
                    </li>
                    <li className="flex items-start gap-2">
                      <span className="text-red-500 mt-0.5">⚠️</span>
                      <span>赎回后不再获得质押收益</span>
                    </li>
                  </ul>
                </div>
              </div>
              ...
              <div className="flex gap-2 mt-2">
                {[25, 50, 75, 100].map((percent) => (
                  <button
                    ...
                    type="button"
                    onClick={() => {
                      // 这里需要获取用户当前持有的 aToken 余额
                      // 暂时设为用户输入的金额百分比
                      const currentAmount = parseFloat(amountIn) || 0;
                      setAmountIn(currentAmount > 0 ? String(currentAmount * percent / 100) : String(percent));
                    }}
