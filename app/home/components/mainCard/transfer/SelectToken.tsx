...
function renderIconLocal(token: string, networkOverride?: string) {
  ...
  const compName = `Token${String(token).replace(/[^A-Za-z0-9]/g, '')}`;
        ...
        const entries = await Promise.all(
          options.map(async (opt) => {
            try {
              ...
              const b = await fetchBalance(address, tokenParam);
  ...
  return (
    <div className="w-full">
      <div className="flex items-center justify-between mb-2">
        <label className="text-sm font-semibold text-gray-800">选择代币</label>
      </div>
